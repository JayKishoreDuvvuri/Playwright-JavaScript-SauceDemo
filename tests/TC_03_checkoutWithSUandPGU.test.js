...
test.describe('Login as a Standard User to select a product item and then complete the checkout workflow with Performance Glitch User', () => {
	test('Login to App as a standard user', async ({
		...
		await test.step(
			...
			async () => {
				await checkoutYourInformationPage.clickCancelBtn()
				await productsPage.verifyProductsPageTitleVisible()
				expect(await checkoutYourInformationPage.getUrl()).toBe(landingPageUrl)
			}
		)
